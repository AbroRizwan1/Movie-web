...
export default function MovieCard({
  ...
  const colorClass =
    genreColors[movie.genre] || "text-white/50 bg-white/5 border-white/10";
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden aspect-[2/3]">
        ...
        {/* Wishlist button */}
        <button className="absolute top-3 left-3 w-8 h-8 rounded-lg bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-white/20">
          <svg
            width="13"
            height="13"
            viewBox="0 0 24 24"
      ...
      <div className="p-4">
        <div className="mb-2">
          <h3 className="text-white font-semibold text-sm leading-tight line-clamp-1 flex-1">
            {movie.title}
          </h3>
        </div>
        ...
        <div className="flex flex-wrap justify-end gap-1.5 pb-1.5">
          {movie.genre_ids.map((id) => {
            ...
            return (
              <span
                key={id}
                className={`flex-shrink-0 text-[7px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                  genreColors[name] ||
